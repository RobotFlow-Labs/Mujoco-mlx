...
def _tree_map(fn, *nodes):
  """Recursive tree-map helper over tuples/lists/dicts/dataclasses."""
  first = nodes[0]
  if isinstance(first, tuple):
    return tuple(_tree_map(fn, *(n[i] for n in nodes)) for i in range(len(first)))
  if isinstance(first, list):
    return [_tree_map(fn, *(n[i] for n in nodes)) for i in range(len(first))]
  if isinstance(first, dict):
    return {k: _tree_map(fn, *(n[k] for n in nodes)) for k in first}
  if dataclasses.is_dataclass(first) and not isinstance(first, type):
    vals = {}
    for f in dataclasses.fields(first):
      vals[f.name] = _tree_map(fn, *(getattr(n, f.name) for n in nodes))
    return dataclasses.replace(first, **vals)
  return fn(*nodes)
...
def _stack_tree(vals):
  ...
  if isinstance(first, dict):
    return {k: _stack_tree([v[k] for v in vals]) for k in first}
  if dataclasses.is_dataclass(first) and not isinstance(first, type):
    fields = {}
    for f in dataclasses.fields(first):
      fields[f.name] = _stack_tree([getattr(v, f.name) for v in vals])
...
def _vmap(fn, in_axes=0):
  ...
  def mapped(*args):
    ...
    for arg, ax in zip(args, axes):
      if ax is not None and arg is not None:
        n = arg.shape[ax]
        break
    if n is None:
      return fn(*args)
...
def _segment_sum(x: mx.array, ids: np.ndarray, size: int) -> mx.array:
  """Loop-based replacement for jax.ops.segment_sum."""
  x_np = np.array(x)
  out = np.zeros((size,) + x_np.shape[1:], dtype=x_np.dtype)
...
def _take(obj: Y, idx: np.ndarray) -> Y:
  ...
  XLA executes x[mx.array([1, 2, 3])] slower than x[1:4], so we detect when
...
def _nvmap(f: Callable[..., Y], *args) -> Y:
  ...
  # split out numpy and array args
  ...
  args = _tree_map(
      lambda a: a if (a is not None and getattr(a, 'shape', (0,))[0]) else None,
      args,
  )
  ...
  return _vmap(outer_f, in_axes=in_axes)(*args)
...
def _check_output(
    y: mx.array, take_ids: np.ndarray, typ: str, idx: int
) -> None:
...
def flat(
  ...
  ys = [
      [v if typ in flat_ else mx.concatenate(v) for v, typ in zip(y, out_types)]
      for y in ys
  ]
  ys = _tree_map(lambda *x: mx.concatenate(x), *ys)
...
def body_tree(
    ...
    if reverse:
      ...
      for child_key in child_keys:
        ...
        def index_sum(x, i=id_map, s=body_ids.size):
          return _segment_sum(x, i, s)

        y = _tree_map(index_sum, y)
        carry = y if carry is None else _tree_map(mx.add, carry, y)
    elif key in key_parents:
      ys = [key_y[p] for p in key_parents[key]]
      y = _tree_map(lambda *x: mx.concatenate(x), *ys)
  ...
  for i, typ in enumerate(out_types):
    ...
    if typ != 'b':
      y_typ = _tree_map(mx.concatenate, y_typ)
    y_typ = _tree_map(lambda *x: mx.concatenate(x), *y_typ)
