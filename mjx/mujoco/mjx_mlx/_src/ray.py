...
def _ray_quad(
  ...
  x0 = mx.where((det < mujoco.mjMINVAL) | (x0 < 0), mx.inf, x0)
  x1 = mx.where((det < mujoco.mjMINVAL) | (x1 < 0), mx.inf, x1)
...
def _ray_plane(
  ...
  p = pnt[0:2] + x * vec[0:2]
  valid &= mx.all((size[0:2] <= 0) | (mx.abs(p) <= size[0:2]))

  return mx.where(valid, x, mx.inf)
...
def _ray_capsule(
  ...
  x = mx.where(mx.abs(pnt[2] + x * vec[2]) <= size[1], x, mx.inf)

  # top cap
  dif = pnt - mx.array([0, 0, size[1]])
  x0, x1 = _ray_quad(vec @ vec, vec @ dif, dif @ dif - size[0] * size[0])
  # accept only top half of sphere
  x = mx.where((pnt[2] + x0 * vec[2] >= size[1]) & (x0 < x), x0, x)
  x = mx.where((pnt[2] + x1 * vec[2] >= size[1]) & (x1 < x), x1, x)
...
def _ray_ellipsoid(
  ...
  # invert size^2
  s = math.safe_div(1, mx.square(size))
...
def _ray_box(
  ...
  iface = mx.array([(1, 2), (0, 2), (0, 1), (1, 2), (0, 2), (0, 1)])

  # side +1, -1
  # solution of pnt[i] + x * vec[i] = side * size[i]
  x = mx.concatenate([math.safe_div(size - pnt,  vec), -math.safe_div(size + pnt, vec)])

  # intersection with face
  p0 = pnt[iface[:, 0]] + x * vec[iface[:, 0]]
  p1 = pnt[iface[:, 1]] + x * vec[iface[:, 1]]
  valid = mx.abs(p0) <= size[iface[:, 0]]
  valid &= mx.abs(p1) <= size[iface[:, 1]]
  valid &= x >= 0

  return mx.min(mx.where(valid, x, mx.inf))
...
def _ray_triangle(
  ...
  nrm = mx.array(np.cross(np.array(vert[0] - vert[2]), np.array(vert[1] - vert[2])))
  dist = math.safe_div(mx.dot(vert[2] - pnt, nrm), mx.dot(vec, nrm))
  valid &= dist >= 0
  dist = mx.where(valid, dist, mx.inf)
...
def _ray_mesh(
  ...
  dists = mx.concatenate(dists)
  min_id = mx.argmin(dists)
  ...
  id_ = mx.array(np.concatenate(geom_ids))[min_id, None]
...
def ray(
  ...
  if not ids:
    return mx.array(-1), mx.array(-1.0)

  dists = mx.concatenate(dists)
  ids = mx.concatenate(ids)
  min_id = mx.argmin(dists)
  dist = mx.where(mx.isinf(dists[min_id]), -1, dists[min_id])
  id_ = mx.where(mx.isinf(dists[min_id]), -1, ids[min_id])

  return dist, id_

def ray_geom(
    size: mx.array, pnt: mx.array, vec: mx.array, geomtype: GeomType
) -> mx.array:
