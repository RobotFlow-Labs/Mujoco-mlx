...
def _apply_cutoff(
  ...
  def fn(sensor, cutoff):
    if data_type == mujoco.mjtDataType.mjDATATYPE_REAL:
      return mx.where(cutoff > 0, mx.clip(sensor, -cutoff, cutoff), sensor)
    elif data_type == mujoco.mjtDataType.mjDATATYPE_POSITIVE:
      return mx.where(cutoff > 0, mx.minimum(sensor, cutoff), sensor)
    else:
      return sensor
...
def sensor_pos(m: Model, d: Data) -> Data:
  ...
  for sensor_type in set(m.sensor_type[stage_pos]):
    ...
    if sensor_type == SensorType.MAGNETOMETER:
      sensor = _vmap(lambda xmat: xmat.T @ m.opt.magnetic)(
          d.site_xmat[objid]
      )
      ...
      def _cam_project(
        ...
        denom = mx.where(
            mx.abs(denom) < mujoco.mjMINVAL,
            mx.clip(denom, -mujoco.mjMINVAL, mujoco.mjMINVAL),
            denom,
        )
      ...
      for ot, rt in set(zip(objtype, reftype)):
        ...
        cutofft = cutoff[idxt]
        sensor = _vmap(_framepos)(xpos, xpos_ref, xmat_ref, refidt)
      ...
      def _frameaxis(xmat, xmat_ref, refid):
        ...
        return mx.where(refid == -1, axis, xmat_ref.T @ axis)
      ...
      for ot, rt in set(zip(objtype, reftype)):
        ...
        xmat_ref = xmat_ref[refidt]
        cutofft = cutoff[idxt]
        sensor = _vmap(_frameaxis)(xmat, xmat_ref, refidt)
        adrt = adr[idxt, None] + np.arange(3)[None]
        sensors.append(_apply_cutoff(sensor, cutofft, data_type[0]).reshape(-1))
        adrs.append(adrt.reshape(-1))
      continue  # avoid adding to sensors/adrs list a second time
      ...
      sensor = mx.repeat(d.time, sum(idx))
    else:
...
def sensor_acc(m: Model, d: Data) -> Data:
  ...
  if (m.sensor_type[stage_acc] == SensorType.TOUCH).any() | (
    ...
    contact_force = mx.concatenate(contact_force)[
    ...
    if sensor_type == SensorType.TOUCH:
      ...
      def _distance(site_size, site_xpos, site_xmat, site_type, pos, conray):
        def dist(size, xpos, xmat, conray):
          ...
          return _vmap(ray_geom_)(pnt, vec)

        return _vmap(dist)(site_size, site_xpos, site_xmat, conray)
      ...
      if contact_maxforce:
        ...
        force_mag = _vmap(
            lambda forcetorque: mx.dot(forcetorque[:3], forcetorque[:3])
        )(contact_force)
      ...
      for dataspec, reduce, objtype, reftype, dim in set(
        ...
        if dataspec & (1 << 0):  # found
          slot.append(mx.repeat(nfound, num)[:, None])
