...
import mujoco
from mujoco.mjx_mlx._src import dataclasses as mjx_dataclasses
...
def _tree_map(fn, *nodes):
  ...
  if len(nodes) == 1:
    ...
    if isinstance(node, tuple):
      return tuple(_tree_map(fn, v) for v in node)
    if isinstance(node, list):
      return [_tree_map(fn, v) for v in node]
    return mjx_dataclasses.tree_map(fn, node)
  ...
  return mjx_dataclasses.tree_map(fn, *nodes)
...
def convex(m: Union[mujoco.MjModel, Model], data_id: int) -> ConvexMesh:
  ...
  return _tree_map(mx.array, c)

def hfield_prism(vert: mx.array) -> ConvexInfo:
  ...
  def get_face_norm(face):
    ...
    edge0 = face[2, :] - face[1, :]
    edge1 = face[0, :] - face[1, :]
    return math.normalize(mx.array(np.cross(np.array(edge0), np.array(edge1))))
